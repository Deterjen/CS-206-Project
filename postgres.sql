...
-- Create a stored procedure for efficiently counting students per university
CREATE OR REPLACE FUNCTION get_university_student_counts()
RETURNS TABLE (university_id INTEGER, count BIGINT) AS $$
BEGIN
    RETURN QUERY
    SELECT es.university_id, COUNT(*) as count
    FROM existing_students es
    GROUP BY es.university_id
    ORDER BY count DESC;
END;
$$ LANGUAGE plpgsql;
...
-- 1. Function to get complete existing student data in a single call
CREATE OR REPLACE FUNCTION get_existing_students_complete(max_records INT DEFAULT 2000)
...
BEGIN
    ...
    SELECT to_jsonb(students) FROM students;
END;
$$ LANGUAGE plpgsql;
...
-- 3. Function to get complete aspiring student data in a single call
CREATE OR REPLACE FUNCTION get_aspiring_student_complete(p_student_id INT)
RETURNS jsonb AS $$
DECLARE
    result jsonb;
BEGIN
    SELECT to_jsonb(complete_student) INTO result
    FROM (
        SELECT
            as1.id,
            as1.created_at,
...
-- 5. Function to get recommendation details in one call
CREATE OR REPLACE FUNCTION get_recommendation_details(p_recommendation_id INT)
RETURNS jsonb AS $$
DECLARE
    recommendation_data jsonb;
    university_data jsonb;
    similar_students_data jsonb;
    result jsonb;
BEGIN
    -- Get recommendation data
    SELECT to_jsonb(r) INTO recommendation_data
    FROM recommendations r
    WHERE r.id = p_recommendation_id;

    -- Get university data
    SELECT to_jsonb(u) INTO university_data
    FROM universities u
    WHERE u.id = (recommendation_data->>'university_id')::int;

    -- Get similar students data as a JSON array
    SELECT jsonb_agg(to_jsonb(student_data)) INTO similar_students_data
    FROM (
        SELECT
            ss.id,
            ss.existing_student_id as student_id,
            ss.similarity_score as overall_similarity,
            ss.academic_similarity,
            ss.social_similarity,
            ss.financial_similarity,
            ss.career_similarity,
            ss.geographic_similarity,
            ss.facilities_similarity,
            ss.reputation_similarity,
            ss.personal_fit_similarity,
            es.university_id,
            u.name as university_name
        FROM
            similar_students ss
        JOIN
            existing_students es ON ss.existing_student_id = es.id
        JOIN
            universities u ON es.university_id = u.id
        WHERE
            ss.recommendation_id = p_recommendation_id
    ) student_data;
    ...
    result = jsonb_build_object(
        ...
        'similar_students', COALESCE(similar_students_data, '[]'::jsonb)
    );
